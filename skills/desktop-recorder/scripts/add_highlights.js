...
const CURSOR_SIZE_OVERRIDE = (() => {
  const raw = readFlag("--cursor-size", null);
  return raw == null ? null : Number(raw);
})();
...
if (!fs.existsSync(ripplePath)) {
  ...
  const C = D / 2;
  // Soft circle: alpha falls off quadratically to a 6 % anti-aliased seam.
...
if (cursorAssets.arrow && cursorClicks.length > 0) {
  ...
  // Hotspot of pointing-hand: fingertip at (12/32, 1/32) of sprite grid.
  const pointingHotspotX = Math.round(cursorSize * 12 / 32);
  const pointingHotspotY = Math.round(cursorSize *  1 / 32);
  ...
  arrowSegs.forEach((seg, i) => {
    ...
    if (seg.interp) {
      ...
      xExpr = `'${seg.x.toFixed(1)}+(${seg.x1.toFixed(1)}-${seg.x.toFixed(1)})*(t-${seg.t0.toFixed(3)})/${dt}'`;
      yExpr = `'${seg.y.toFixed(1)}+(${seg.y1.toFixed(1)}-${seg.y.toFixed(1)})*(t-${seg.t0.toFixed(3)})/${dt}'`;
    } else {
      xExpr = seg.x.toFixed(1);
      yExpr = seg.y.toFixed(1);
    }
    chain.push(
  ...
  cursorClicks.forEach((c, i) => {
    ...
    const nxt = `[pc${i}]`;
    const px = (c.x - pointingHotspotX).toFixed(1);
    const py = (c.y - pointingHotspotY).toFixed(1);
    chain.push(
...
args.push(
  ...
  "-movflags", "+faststart",
  // Bounds the output to the shortest finite input (the video); without it
  // the looped -loop 1 PNG inputs never EOS and ffmpeg writes forever.
  "-shortest",
  "-an",
  OUT,
);
